...
IMoveable::IMoveable() : linearDampingCoefficient(default_linearDampingCoefficient), angularDampingCoefficient(default_angularDampingCoefficient)
{	
	try
	{
		Pose = new se3();
...
IMoveable::IMoveable( const se3& Pose_): linearDampingCoefficient(default_linearDampingCoefficient), angularDampingCoefficient(default_angularDampingCoefficient)
{	
	try
	{
		Pose = new se3();
...
//Orientation has to be initialized from Pose.
IMoveable::IMoveable( const se3& Pose_, const Mat<float>& Lvel, const Mat<float>& Avel): linearDampingCoefficient(default_linearDampingCoefficient), angularDampingCoefficient(default_angularDampingCoefficient), Pose( new se3() ), LinearVelocity( new Mat<float>(Lvel)), AngularVelocity( new Mat<float>(Avel))
{
	*Pose = Pose_;
	//Orientation = Qt_FromMat( extract(Pose.exp(), 1,1,3,3) );	
}	
